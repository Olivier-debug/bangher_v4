...
// Central wiper (must support: wipeAll({required supa, bool clearWebLocalStorage}))
...
class _SettingsPageState extends State<SettingsPage> {
  ...
  // ---- Discovery prefs (UI only for this page)
  ...
  // ---------------------------------------------------------------------------
  // RESET CACHE (while staying logged in): DO NOT clear auth storage here.
  Future<void> _resetCache() async {
    ...
    showDialog(
      ...
      builder: (_) => const _SigningOutDialog(), // small spinner UI
    );

    try {
      // Preserve session on web: clearWebLocalStorage=false
      await CacheWiper.wipeAll(supa: _supa, clearWebLocalStorage: false);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  // ---------- CLEAN, FAST LOGOUT ----------
  // Only resets local *UI* state here. No storage wiping.
  Future<void> _preLogoutCleanupUiOnly() async {
    ...
    // tiny pause to let the UI breathe (optional)
  ...
  Future<void> _logout() async {
    ...
    try {
      // 1) UI reset only (no storage wiping yet)
      await _preLogoutCleanupUiOnly();

      // 2) Sign out first — this removes the session from secure/local storage.
      await _supa.auth.signOut();

      // 3) Post-logout: now it's safe to fully nuke caches + web localStorage.
      await CacheWiper.wipeAll(supa: _supa, clearWebLocalStorage: true);
    } catch (_) {
      // Ignore sign-out/wipe errors; we'll still navigate.
    } finally {
      if (mounted) {
        // 4) Dismiss spinner
        final rootNav = Navigator.of(context, rootNavigator: true);
        ...
        // 5) Navigate AFTER the pop completes to avoid _debugLocked
