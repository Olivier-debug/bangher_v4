...
// - NEW: Send `fresh=1` ONLY for register flows (email signup + OAuth from Create tab).

import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:flutter/foundation.dart' show kIsWeb, debugPrint; // OAuth redirect + debugPrint
import 'package:google_fonts/google_fonts.dart';
import 'package:font_awesome_flutter/font_awesome_flutter.dart';
import 'package:supabase_flutter/supabase_flutter.dart';
import 'package:go_router/go_router.dart';
import 'package:shared_preferences/shared_preferences.dart'; // <-- for OAuth fresh intent flag
...
// ---- Local flag key for OAuth "fresh" intent (Create tab only) ----
...
class LoginPageWidgetState extends State<LoginPageWidget> with TickerProviderStateMixin {
  ...
  // NEW: context-free navigation helper to avoid `use_build_context_synchronously`
  Future<void> _goToCreateOrComplete({
    required GoRouter router,
    required NavigatorState navigator,
    bool fresh = false,
  }) async {
    final qp = fresh ? {'fresh': '1'} : const <String, String>{};
    try {
      router.goNamed(
        kRouteCreateOrCompleteName,
        queryParameters: qp,
        extra: {'fresh': fresh}, // robustness for router builders that read `extra`
      );
      return;
    } catch (_) {
      // fall through
    ...
    try {
      navigator.pushReplacementNamed(
        fresh ? '$kRouteCreateOrCompletePath?fresh=1' : kRouteCreateOrCompletePath,
      );
    } catch (e) {
      debugPrint(
        'Navigation error: "$kRouteCreateOrCompleteName" / "$kRouteCreateOrCompletePath" not found. $e',
      );
    }
  }
  ...
  // ---- Little helpers to mark OAuth "fresh" intent (Create tab only)
  Future<void> _markFreshIntent() async {
    final p = await SharedPreferences.getInstance();
    await p.setBool(kFreshIntentKey, true);
  }

  // ---- DB helper to avoid ON CONFLICT error when unique index isn't ready
  Future<void> _ensureRowByUserId(String table, String userId) async {
    ...
    try {
      ...
      // 42P10: no unique/exclusion constraint for ON CONFLICT
      if (e.code == '42P10') {
        ...
        if (existing == null) {
          try {
            ...
            // 23505 duplicate key (race) → ignore
  ...
  // -------------------- AUTH ACTIONS --------------------
  // Create tab: Email/password signup → always fresh.
  Future<void> _handleCreateAccount() async {
    ...
    // Capture navigation handles BEFORE any awaits to avoid using context later.
    ...
    try {
      ...
      // Create skeleton rows safely (won’t crash without unique indexes)
      await _ensureRowByUserId('profiles', userId);
      await _ensureRowByUserId('preferences', userId);

      // Navigate WITHOUT using BuildContext after awaits
      await _goToCreateOrComplete(
        router: router,
        navigator: navigator,
        fresh: true, // fresh ONLY here
      );
  ...
  // Login tab: Email/password sign-in → **do not navigate**.
  // Let GoRouter’s redirect (listening to the auth stream) decide:
  // - If profile is complete → it pushes /swipe
  // - If not complete → it pushes /create-or-complete-profile
  Future<void> _handleSignIn() async {
    ...
    try {
      ...
      // No explicit navigation here; router handles it via auth stream.
    } on AuthException catch (e) {
      _showSnack(e.message);
    } catch (e) {
  ...
  // Create tab: OAuth → mark fresh intent BEFORE launching OAuth
  Future<void> _handleGoogleSignUp() async {
    if (_isOauthLaunching) return;
    setState(() => _isOauthLaunching = true);
    try {
      await _markFreshIntent(); // <-- signal for router to append ?fresh=1 after OAuth
  ...
  // Login tab: OAuth → DO NOT mark fresh
                                        ...
                                        child: SlideTransition(
                                          ...
                                          child: FadeTransition(
                                            ...
                                            child: CreateAccountForm(
                                              ...
                                              onGoogle: _handleGoogleSignUp,            // OAuth signup → fresh
                                        ...
                                        child: SlideTransition(
                                          ...
                                          child: FadeTransition(
                                            ...
                                            child: LoginForm(
                                              ...
                                              onSubmit: _handleSignIn,                  // email login → NOT fresh (no navigation)
