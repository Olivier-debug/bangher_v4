...
class LoginPageWidgetState extends State<LoginPageWidget> with TickerProviderStateMixin {
  ...
  Future<void> _goToCreateOrComplete({bool fresh = false}) async {
    if (!mounted) return;
  ...
  Future<void> _handleCreateAccount() async {
    ...
    try {
      ...
      await _goToCreateOrComplete(fresh: true); // <-- fresh ONLY here
  ...
  Future<void> _handleSignIn() async {
    ...
    try {
      ...
      // ✅ IMPORTANT CHANGE:
      // Do NOT navigate here. Your router’s `refreshListenable` will fire on `signedIn`,
      // run the redirect while we’re still on /loginPage, and send the user to the right place.
      // (Previously this called `_goToCreateOrComplete();` which could cause a flash.)
      // No-op on purpose.
    } on AuthException catch (e) {
      _showSnack(e.message);
    } catch (e) {
