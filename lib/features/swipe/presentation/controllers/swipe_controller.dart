...
// Adds: recordSwiped() + prune() so compaction logic has signal + bounded memory.
// ============================================================================
...
class SwipeController extends StateNotifier<SwipeUiState> {
  ...
  Future<void> swipeCard({
    ...
    try {
      ...
      cache.recordSwiped(swipeeId); // enable deterministic pruning/compaction
      await repo.swipe(swipeeId: swipeeId, liked: liked);
      cache.removePending(swipeeId);
      cache.removeUnswipeOverride(swipeeId);
      cache.prune(maxSwiped: 6000, maxPending: 512); // bound memory
