...
// Added: utilities to paint/prefetch the TOP card first.
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// TOP-FIRST LOADING/PRESENTATION UTILITIES
// ─────────────────────────────────────────────────────────────────────────────

/// Returns children so that the *top* card is built/painted FIRST.
/// Use this when building your Stack of swipe cards:
/// 
/// ```dart
/// final children = stackChildrenTopFirst(cardWidgets);
/// return Stack(children: children);
/// ```
///
/// Rationale: Flutter builds/paints children in list order. If the top-most
/// card is last in the Stack, its images kick off last, causing back cards to
/// appear first. This reorders so top card starts loading immediately.
List<Widget> stackChildrenTopFirst(List<Widget> cardWidgets) {
  // Your deck is usually built bottom→top; reverse to paint top first.
  final copy = List<Widget>.from(cardWidgets);
  return copy.reversed.toList(growable: false);
}

/// Prefetch the first `count` images of the *top* card to make it pop in fast.
/// Call this as soon as you set new `SwipeUiState(cards: ...)`.
///
/// ```dart
/// await prefetchTopCardImages(context, state, count: 2);
/// ```
///
/// Uses `CachedNetworkImageProvider` so it benefits from its cache.
...
/// Positioned.fill, but as a widget for brevity.
