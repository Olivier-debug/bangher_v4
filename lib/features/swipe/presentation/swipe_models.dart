// ─────────────────────────────────────────────────────────────────────────────
// FILE: lib/features/swipe/presentation/swipe_models.dart
// ─────────────────────────────────────────────────────────────────────────────

import 'dart:async' show TimeoutException;
// needed for transparentPixel
...
final Uint8List transparentPixel = Uint8List.fromList(<int>[
  ...
  0, 0, 1, 8, 6, 0, 0, 0, 31, 21, 196, 137, 0, 0, 0, 1, 115, 82, 71, 66, 0,
  174, 206, 28, 233, 0, 0, 0, 10, 73, 68, 65, 84, 8, 153, 99, 0, 1, 0, 0, 5, 0,
  1, 13, 10, 44, 170, 0, 0, 0, 0, 73, 69, 78, 68, 174, 66, 96, 130
]);
...
class SwipeCard {
  ...
  factory SwipeCard.fromJson(Map<String, dynamic> m) {
    List<String> listOfString(dynamic v) => (v as List? ?? const [])
        .map((e) => e?.toString() ?? '')
...
class SwipeResult {
  ...
  final String? matchId; // optional; parsed when backend provides it
  ...
  factory SwipeResult.fromJson(Map<String, dynamic>? m) {
    ...
    // why: accept multiple server spellings without breaking callers
    final dynamic idRaw = m['match_id'] ?? m['matchId'] ?? m['matchid'];
    return SwipeResult(
      ...
      matchId: (idRaw == null || idRaw.toString().isEmpty) ? null : idRaw.toString(),
    );
  }
}
...
class RetryPolicy {
  ...
  static bool defaultPredicate(Object e) {
    ...
    if (e is PostgrestException) {
      ...
      if (msg.contains('timeout') || msg.contains('connection') || msg.contains('terminating connection')) {
        return true;
      }
      if (msg.contains('permission denied') || msg.contains('violates') || msg.contains('invalid')) {
        return false;
      }
      return true;
    }
...
List<Widget> stackChildrenTopFirst(List<Widget> cardWidgets) {
  final copy = List<Widget>.from(cardWidgets);
  return copy.reversed.toList(growable: false);
}

Future<void> prefetchTopCardImages(
  BuildContext context,
  SwipeUiState state, {
  int count = 2,
}) async {
  if (state.cards.isEmpty) return;
  final top = state.cards.first;
  final urls = top.photos.take(count);
...
class PositionedFill extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // keep layout stable; replace only child
    return const Positioned(left: 0, top: 0, right: 0, bottom: 0, child: SizedBox.shrink())
        .copyWith(child: child);
  }
}

extension on Positioned {
  Positioned copyWith({Widget? child}) =>
      Positioned(left: left, top: top, right: right, bottom: bottom, child: child ?? this.child);
}
