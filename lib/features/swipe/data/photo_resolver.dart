...
// Hardened resolver: re-sign expired sign-URLs, robust stable keys, tolerant parsing.
...
class ResolvedPhoto {
  ...
  final String cacheKey;  // stable cache key (no querystring, or bucket/path)
...
class PhotoResolver {
  ...
  /// kind ∈ {public, sign, authenticated}
  (String bucket, String objectPath)? _parseBucketAndPathFromUrl(String url) {
    ...
    // If there's a kind segment, bucket is after it; otherwise assume next is bucket.
    String bucket;
    List<String> rest;
  ...
  /// Signed URL typical query has `token` and `expires` (or `exp`).
  bool isSignedAndExpired(String url) {
    final uri = Uri.tryParse(url);
    if (uri == null) return false;
    if (!uri.queryParameters.containsKey('token')) return true; // definitely unusable
  ...
  Future<ResolvedPhoto> resolvePhoto(String raw) async {
    ...
    if (memo != null) {
      // If using signed URLs and it expired, fall through to re-resolve.
    ...
    if (_isHttp(raw)) {
      ...
      // Use as-is; stable key by bucket/path when possible, else drop query.
