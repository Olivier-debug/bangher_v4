...
class PhotoResolver {
  ...
  PhotoResolver(this._supa, {bool useSignedUrls = true})
      : _useSignedUrls = useSignedUrls;
  ...
  bool _looksLikeSupabaseUrl(String s) {
    ...
    return u != null &&
        u.host.contains('.supabase.co') &&
        u.path.contains('/storage/v1/object/');
  }
  ...
  bool _hasQueryToken(String s) {
    ...
    return u != null &&
        (u.queryParameters.containsKey('token') ||
            u.queryParameters.containsKey('t'));
  }
  ...
  (String bucket, String objectPath)? _parseBucketAndPathFromStorageScheme(
      String input) {
  ...
  Future<ResolvedPhoto> _sign(
    String bucket,
    String path,
    String stableKeyIfNeeded,
  ) async {
    final normalizedPath = _trimLeadingSlash(path);
    final url = _useSignedUrls
        ? await _supa.storage
            .from(bucket)
            .createSignedUrl(normalizedPath, 55 * 60)
        : _supa.storage.from(bucket).getPublicUrl(normalizedPath);
    final key = _useSignedUrls
        ? '$bucket/$normalizedPath'
        : (stableKeyIfNeeded.isEmpty ? '$bucket/$normalizedPath' : stableKeyIfNeeded);
  ...
  String cacheKeyForUrl(String url) =>
      _stableKeyByUrl[url] ?? _stableKeyFromUrl(url);
  ...
  /// Keep it simple: WEBP everywhere (broad support).
