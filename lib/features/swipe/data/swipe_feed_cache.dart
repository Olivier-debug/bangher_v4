// =========================
// FIXED FILE: swipe_feed_cache.dart
// =========================
...
class SwipeFeedCache {
  ...
  /// READ-ONLY: current key (for debugging/inspection)
  String? get currentKey => _key;

  /// Check if the given key matches current key.
  bool isCurrentKey(String key) => _key == key;

  /// Unmodifiable view of raw list (mutable internally for trim/compaction).
  List<Map<String, dynamic>> get cards => List.unmodifiable(_cards);

  /// Projection for UI: backing cards minus locally-swiped ids (unmodifiable).
  List<Map<String, dynamic>> get visibleCards {
    if (_cards.isEmpty) return const <Map<String, dynamic>>[];
    if (swipedIds.isEmpty) return List.unmodifiable(_cards);
  ...
  void _compactIfNeeded({
    ...
    // LRU eviction: keep only the most recent (end of list)
    if (next.length > hardCap) {
      next.removeRange(0, next.length - hardCap);
    } else if (next.length > softCap) {
