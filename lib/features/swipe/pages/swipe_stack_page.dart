...
// (UNCHANGED except for your existing additions; reprinted for completeness)
...
class _TestSwipeStackPageState extends ConsumerState<TestSwipeStackPage>
  ...
  String? _visibleMatchOverlayId; // tracks currently-open match overlay
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _matchSub = ctrl.matchStream.listen((e) async {
        ...
        // Suppress if DB says we've already seen this match.
        ...
        // Enrich from deck when possible
        ...
        _visibleMatchOverlayId = e.matchId;            // ← BEGIN overlay tracking
        try {
          ...
          // Fallback ensures "seen" even if the route changed while open
  ...
  void dispose() {
    ...
    // Fallback: component disposed while overlay visible
    final mid = _visibleMatchOverlayId;
    if (mid != null) {
      final meId = Supabase.instance.client.auth.currentUser?.id;
      if (meId != null && meId.isNotEmpty) {
        // fire-and-forget; we are in dispose()
  ...
  void didPushNext() {
    ...
    // Fallback: user navigated while match overlay visible
    final mid = _visibleMatchOverlayId;
    if (mid != null) {
      final meId = Supabase.instance.client.auth.currentUser?.id;
      if (meId != null && meId.isNotEmpty) {
...
Future<void> _applyRadiusAndReload(double km) async {
                          ...
                          if (matchId.isNotEmpty && _shownMatchIds.add(matchId)) {
                            ...
                            if (meId.isNotEmpty) {
                              ...
                              if (!alreadySeen) {
                                ...
                                _visibleMatchOverlayId = matchId;      // ← track open
                                try {
                                  await overlay.MatchOverlay.show(
                                    ctx,
                                    me: meLite,
  ...
  // Small helper to keep _card readable (no behavior change).
                        ...
                        children: [
                          ...
                          if (p > 0)
                            const IgnorePointer(
                              child: Opacity(
                                opacity: 0.06,
                                child: DecoratedBox(decoration: BoxDecoration(color: Colors.cyan)),
                              ),
                            ),
                        ],
                      ),
