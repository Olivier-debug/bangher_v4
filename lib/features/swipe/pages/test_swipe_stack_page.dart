...
//   New gates: `_showSkeleton` while refilling, `_showEmpty` only when not
//   fetching and stack truly empty.
...
// - **API**: Immediate local consume + outbox; RPC then direct fallback.
...
class _TestSwipeStackPageState extends State<TestSwipeStackPage>
  ...
  void initState() {
    ...
    _stack.addListener(() {
      ...
      if (mounted) setState(() {}); // keep `_hasCurrentCard` reactive
    });
  ...
  // EMPTY only when not fetching/initializing and no cards exist to show.
  ...
  void _processSwipe({required int index, required SwipeDirection direction}) {
    ...
    if (remainingAfterThis <= _topUpThreshold && !_cache.exhausted) {
      if (remainingAfterThis == 0 && mounted) {
        setState(() => _fetching = true); // ensure skeleton instead of blank
      }
      unawaited(_loadBatch(wanted: _topUpBatch));
    }
  ...
  Widget _buildStackAndMeasure() {
    return LayoutBuilder(
      builder: (context, box) {
        ...
        // Mobile best-practice: ~92–94% of width, clamped for elegance.
                ...
                return GestureDetector(
                  ...
                  child: Stack(
                    children: [
                      ...
                      // Status chip (Active / Recently active / Last seen X)
                      Positioned(
                        top: 12,
                        left: 34,
                        child: _StatusChip(
  ...
  // Bottom bar sized to card width. Buttons lowered + safe-area padding.
  Widget _bottomBar() {
...
_PresenceInfo _presenceInfo({required bool isOnline, required dynamic lastSeenRaw}) {
  ...
  // Recently active threshold: 10 minutes
  if (minutes <= 10) {
    return const _PresenceInfo(
      ...
      child: url == null
          ? Center(
              child: Text(
                fallbackLetter,
                style: const TextStyle(
                    color: Colors.white70, fontSize: 56, fontWeight: FontWeight.w700),
              ),
            )
          : null,
    );
    ...
    return AnimatedBuilder(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: const Color(0xFF3C4046)),
          boxShadow: [
            BoxShadow(color: Colors.black.withValues(alpha: _kDefaultAlpha), blurRadius: 10)
          ],
        ),
