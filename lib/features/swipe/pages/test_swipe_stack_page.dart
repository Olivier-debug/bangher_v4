...
//   Cause: `_showEmpty` relied on `_cardsLeft()==0`, which counted the top card
//   as already consumed. Fix: `_hasCurrentCard` + new `_showEmpty` condition.
// - Keep immediate swipe persistence but avoid pre-emptive empty-page flash.
// - **UI**: Make the SwipeStack card **wide** (mobile best-practice ~92–94% of width).
//   Bottom bar auto-sizes to card width.
// - **Empty CTA page**: Beautiful “You’re all caught up” with actions:
//   • View already-swiped users (sheet)  • Adjust filters (inline sheet)
// - Filters sheet persists to DB + refreshes feed (resets cache key & reloads).
// - Everything else stays compatible with the existing cache/outbox logic.
...
class _TestSwipeStackPageState extends State<TestSwipeStackPage>
  ...
  void initState() {
    ...
    _stack.addListener(() {
      ...
      // Hint the next prewarm when index advances.
      _warmTopCard();
      _updateFinishedState();
      setState(() {}); // keep `_hasCurrentCard` reactive
    });
  ...
  // Cards strictly AFTER the current top card.
  ...
  // EMPTY is only when feed is exhausted AND there is no current card to show.
  bool get _showEmpty => !_initializing && _cache.exhausted && !_hasCurrentCard;
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          } else if (!_hasCurrentCard && _cards.isEmpty) {
  ...
  Future<int> _loadBatchRpc({required int limit}) async {
    try {
      ...
      _cache.rpcOffset += limit; // advance to avoid re-fetching same rows
      return added;
    } catch (e) {
  ...
  Future<void> _undoLast() async {
    ...
    if (_undoMemory.card != null) {
      ...
      final at =
          (_stack.currentIndex < 0) ? 0 : _stack.currentIndex.clamp(0, _cards.length).toInt();
  ...
  // Marks the feed exhausted when the user has finished the stack and
  // there are no more cards locally (prevents stale stack showing last card swiped).
  ...
  Widget _buildStackAndMeasure() {
    return LayoutBuilder(
      builder: (context, box) {
        ...
        // Mobile best-practice: card takes ~92–94% of width, centered.
        final double cardW = (size.width * 0.94).clamp(280.0, size.width);
        final double cardH = size.height; // let height fill available
        ...
        return Center(
          child: SizedBox(
            ...
            child: SwipableStack(
              ...
              // already handled the swipe in onWillMoveNext.
  ...
  Future<void> _applyFilters({
    ...
    // Reset cache key and reload feed
    ...
    return IgnorePointer(
      child: AnimatedBuilder(
        ...
        builder: (_, __) {
          ...
          (0.35 + 0.35 * (1.0 - t)).clamp(0.0, 0.70);
          return Container(
            width: r,
            height: r,
            decoration: BoxDecoration(
    ...
    return AnimatedBuilder(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(
                color: Colors.black.withValues(alpha: _kDefaultAlpha), blurRadius: 10)
          ],
        ),
