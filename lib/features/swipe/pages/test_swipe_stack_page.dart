...
class _TestSwipeStackPageState extends State<TestSwipeStackPage>
  ...
  void _compactSwipedExceptLast() {
    ...
    // mutate the backing list safely while not mounted
    ...
    // We DO NOT clear idSet inside cache on purpose: strong de-dupe should remain.
  }
  ...
  Future<void> _undoLast() async {
    ...
    // Rewind (only works if the card is still present — we kept it in cache).
    if (_stack.canRewind) {
      try {
        _stack.rewind();
      } catch (e) {
          ...
          children: [
            SwipableStack(
              ...
              onSwipeCompleted: (index, direction) async {
                ...
                // Top-up when <= 5 remain (append-only, no UI hiccup)
                final left = _cardsLeft();
                if (left <= _topUpThreshold && !_cache.exhausted) {
                  unawaited(_loadBatch(wanted: _topUpBatch));
                }
                ...
                if (next < _cards.length) {
                  final photos =
                      (_cards[next]['photos'] as List?)?.cast<String>() ??
                          const [];
                  if (photos.isNotEmpty) {
