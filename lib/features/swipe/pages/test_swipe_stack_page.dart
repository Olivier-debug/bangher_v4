...
// - Preserves public API, class names, fields, and routing.
// - Adds robust end-of-stack detection: shows empty-state when the user
//   has consumed all cards AND no new batch is available.
// - Avoids jarring full-screen loader during top-ups; only shows it when the
//   visible stack is empty (initial load or after consuming last card).
// - Adds RPC paging via _rpcOffset to prevent refetching the same rows.
// - _loadBatch now returns the number of cards added and centralizes state flags.
// - Refactors top-up triggers to be idempotent + race-safe with _fetching guard.
// - Keeps single-level undo behavior and the swiping UX intact.
...
class _TestSwipeStackPageState extends State<TestSwipeStackPage>
  ...
  bool _exhausted = false; // true when no more data server-side
  ...
  // RPC paging (prevents offset=0 re-fetch loops)
  int _rpcOffset = 0;
  ...
  Future<void> _bootstrap() async {
    ...
    // Show loader for the very first fetch only.
    _showLoader();
  ...
  // ───────────────────────────── Derived helpers
  int _cardsLeft() {
    // Why: currentIndex may move beyond last index; clamp to 0.
    final int consumed = (_stack.currentIndex + 1);
    final int left = _cards.length - consumed;
  ...
  Future<int> _loadBatch() async {
    // Why: avoid concurrent fetches and avoid jarring loader while cards remain visible.
    if (_fetching) return 0;

    final bool needFullLoader = _cardsLeft() == 0 && !_loaderVisible;

    setState(() => _fetching = true);
    if (needFullLoader) _showLoader();
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          // If we added something, we're not exhausted; if we added nothing and there
          // are no visible cards left, mark exhausted to show empty-state.
          if (added > 0) {
            _exhausted = false;
  ...
  Future<int> _loadBatchRpc() async {
    try {
      ...
      final res = await _supa.rpc(_rpcGetMatches, params: {
        ...
        'limit_arg': _rpcBatch,
        'offset_arg': _rpcOffset, // page forward each call
      });
      ...
      // Advance offset regardless of local filtering to avoid re-hitting the same rows.
      _rpcOffset += _rpcBatch;
      return added;
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    // Empty when: all visible cards consumed AND server reports exhausted.
    final bool showEmpty = _exhausted && _cardsLeft() == 0;
      ...
      children: [
        ...
        Expanded(
          child: Padding(
            ...
            child: showEmpty ? _emptyState() : _buildStackAndMeasure(),
          ),
        ),
        _bottomBar(),
      ],
          ...
          children: [
            SwipableStack(
              ...
              onSwipeCompleted: (index, direction) async {
                ...
                if (remaining <= 0) {
                  // End reached → attempt to fetch next page; show empty-state if none.
                  unawaited(_loadBatch());
                } else if (remaining < _minTopUp) {
                  unawaited(_loadBatch());
                }
