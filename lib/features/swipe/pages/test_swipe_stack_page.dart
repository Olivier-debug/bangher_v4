...
//   cards. Undo keeps a full snapshot in _UndoMemory (no ghost card on return).
// - Strong de-dupe on merge against cache.ids + cache.swipedIds.
// - Per-user photo index keyed by userId to avoid “photo jumping”.
// - Presence + image pre-warm preserved (uses global PresenceService).
// - Swipes are registered immediately in onWillMoveNext (no animation delay).
// - Global outbox guarantees DB persistence; flushes on bootstrap/connectivity.
// - NEW (auth): No proactive refreshes. We rely on Supabase auto-refresh.
//   If a request fails with 401/JWT-expired, we dedupe a single refresh and
//   retry the request once (industry-standard for mobile apps).
...
// Global presence channel (you created this)
import '../../../services/presence_service.dart';

/// Minimal, deduped refresh gate used only after a 401/JWT-expired failure.
/// We do NOT refresh proactively; Supabase auto-refresh handles the rest
/// (ensure `autoRefreshToken: true`, `persistSession: true` at app init).
class _AuthRefreshGate {
  static Future<void>? _inflight;
  static DateTime? _lastRefreshAt;
  static const Duration _minGap = Duration(seconds: 10);

  static Future<void> refreshOnce(SupabaseClient supa) {
    ...
    // Too soon since the last refresh — avoid bursts.
    if (_lastRefreshAt != null && now.difference(_lastRefreshAt!) < _minGap) {
      return _inflight ?? Future.value();
    }

    // Run a single refresh wrapped in an async void-future.
    _inflight = (() async {
      try {
        await supa.auth.refreshSession(); // returns AuthResponse (ignored)
      } catch (_) {
        // swallow; original request will surface its own error if retry fails
      } finally {
        _lastRefreshAt = DateTime.now();
        // mark gate idle; callers holding this Future won't be affected
        _inflight = null;
      }
    })();
...
/// Wrap a Supabase call. If it fails with an auth issue (401/JWT expired),
/// refresh once (deduped) and retry the call a single time.
Future<T> _withAuth<T>(
  ...
  try {
    ...
    if (_looksLikeAuthExpiry(e)) {
      await _AuthRefreshGate.refreshOnce(supa);
      // one retry only
      return await run();
    }
    rethrow;
...
bool _looksLikeAuthExpiry(Object e) {
  final s = e.toString().toLowerCase();
  // Covers Postgrest 401, GoTrue 'jwt expired', 'invalid token', etc.
  return s.contains('401') ||
      s.contains('jwt') && (s.contains('expire') || s.contains('invalid')) ||
      s.contains('invalid authentication') ||
...
class _TestSwipeStackPageState extends State<TestSwipeStackPage>
  ...
  // Presence (using global channel; we poll the state to avoid leak-prone listeners)
  ...
  // Compact swiped profiles on tab switch (no ghost card kept in cache).
  @override
  void deactivate() {
    super.deactivate();
  ...
  Future<void> _bootstrap() async {
    ...
    // Start presence after first frame so it doesn't stack requests on boot
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _startPresence();
    });

    // Minimal, sequential bootstrap (keeps network tame)
    await _loadMyPhoto();
    ...
    if (me != null) {
      final key =
          '$me|g=$_prefGender|a=$_prefAgeMin-$_prefAgeMax|r=$_prefRadiusKm';
      _cache.resetIfKeyChanged(key);
    }

    // Try to flush any pending swipes (outbox) ASAP.
  ...
  void dispose() {
    _connSub?.cancel();
    _presenceTimer?.cancel(); // global channel persists; we only stop our poller
    _stack.dispose();
    super.dispose();
  }
  ...
  // ───────────────────────────── Presence (use global channel safely)
  void _startPresence() {
    final me = _supa.auth.currentUser?.id;
    if (me == null) return;

    final ch = PresenceService.instance.channel(); // global, already subscribed
    // Track (idempotent; Realtime ignores duplicate track payloads)
    unawaited(ch.track({
      'user_id': me,
      'online_at': DateTime.now().toUtc().toIso8601String(),
    }));

    // Poll the presence state every few seconds to update badges
    _presenceTimer?.cancel();
    _presenceTimer = Timer.periodic(const Duration(seconds: 4), (_) {
      final states = ch.presenceState(); // snapshot
      final set = <String>{};
  ...
  Future<int> _loadBatchRpc({required int limit}) async {
    try {
      ...
      final res = await _withAuth(
        _supa,
        () => _supa.rpc(_rpcGetMatches, params: {
          'user_id_arg': me,
  ...
  // Handle swipe IMMEDIATELY (called from onWillMoveNext)
  void _processSwipe({required int index, required SwipeDirection direction}) {
    ...
    // Consume locally NOW to prevent reappearance; queue for persistence.
    _cache.consumeById(id);

    // Snapshot for robust Undo after compaction.
  ...
  Future<void> _recordSwipe({
    ...
    try {
      ...
      // pending remains queued for future flush
    } finally {
  ...
  Future<bool> _sendSwipeToServer({
    ...
    try {
      return await _withAuth<bool>(_supa, () async {
        try {
  ...
  Future<void> _undoLast() async {
    ...
    _cache.unconsumeById(last.swipeeId);

    if (_undoMemory.card != null) {
      _cache.removeById(last.swipeeId);
      final at =
          (_stack.currentIndex < 0) ? 0 : _stack.currentIndex.clamp(0, _cards.length);
  ...
  Future<void> _checkAndShowMatch(String otherUserId) async {
    ...
    try {
      ...
      final existing = await _withAuth(
        _supa,
        () => _supa
            .from('matches')
            .select('id')
        ...
        return Stack(
          children: [
            SwipableStack(
              ...
              // Register swipe immediately here.
              ...
              // Kept for safety; already handled the swipe in onWillMoveNext.
              ...
              builder: (context, props) {
                ...
                final data = _cards[i];
                final userId =
                    data['potential_match_id']?.toString() ?? 'row-$i';
  ...
  Widget _card(
    ...
    final int age =
        (data['age'] is int) ? data['age'] as int : int.tryParse('${data['age']}') ?? 0;
...
class _PulsingHeart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (_, child) {
        final s =
            0.9 + 0.1 * (1 + math.sin(controller.value * math.pi * 2)) / 2;
