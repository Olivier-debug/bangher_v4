...
// with pinned photo caching for full offline image display.
...
class MyProfileStore extends AsyncNotifier<UserProfile?> {
  ...
  Future<void> setPhotos(List<String> urls, {bool requireOnline = false}) async {
    await updateProfile({'profile_pictures': urls}, requireOnline: requireOnline);
  }

  /// Return local file paths for the current profile's photos.
  /// If [ensurePrefetch] is true, this will download missing images.
  ...
  Future<void> _removeFromOutbox(int createdAt) async {
    final xs = _readOutbox();
    xs.removeWhere((e) => e.createdAt == createdAt);
  ...
  UserProfile? _userProfileFromMap(Map<String, dynamic> m) {
    try { return UserProfile.fromMap(m); } catch (_) { return null; }
  }
}
