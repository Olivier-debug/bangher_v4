...
// - Auth-safe: no manual token refreshes; no network while signed-out
// - Coalesced refresh/flush to avoid repeated requests
// - Per-user cache binding: cache is tied to the UID, purged on user change
//
...
const _kLastUidKey = 'my_profile_last_uid'; // NEW: bind cache to user
...
class MyProfileStore extends AsyncNotifier<UserProfile?> {
  ...
  // auth watcher (sign-out cleanup / signed-in bootstrap)
  ...
  Future<UserProfile?> build() async {
    ...
    // Only serve cache when it's owned by the current UID.
    if (uid != null && lastUid == uid) {
      final cached = _readFromDisk();
      state = AsyncData(cached);
    } else {
      // If signed out or user changed, clear any stale local data fast.
      await clearLocal();
    ...
    if (!_bootstrapped) {
      _bootstrapped = true;
      _startAuthListener();        // cheap; needed to react to sign-in/out
      _startConnectivityListener(); // cheap; actual work gated by signed-in checks
      if (uid != null) {
        _startProfileStream();
  ...
  Future<void> updateProfile(
    ...
    _writeRawToDisk(nextRaw, uid: uid); // bind cache to current user
    _raw = nextRaw;
    ...
    if (requireOnline && _outboxContains(action.createdAt)) {
      if (prevRaw == null) {
        ...
        _writeRawToDisk(prevRaw, uid: uid); // keep uid binding
        _raw = prevRaw;
  ...
  Future<void> clearLocal() async {
    ...
    await _prefs?.remove(_kLastUidKey); // ensure no uid leakage
  }
  ...
  void _writeRawToDisk(Map<String, dynamic> raw, {String? uid}) {
    ...
    if (uid != null) {
      _prefs?.setString(_kLastUidKey, uid); // bind cache to this user
    }
  }
  ...
  Future<void> _refreshFromServer() async {
    ...
    // Coalesce / throttle refreshes to avoid bursts.
    ...
    try {
      ...
      final localUpdated =
          DateTime.tryParse(_prefs?.getString(_kMyProfileUpdatedAtKey) ?? '');
      ...
      if (acceptServer) {
        ...
        _writeRawToDisk(raw, uid: uid); // keep uid binding fresh
        _raw = raw;
  ...
  void _startProfileStream() {
    ...
    _profileStream = _db
      ...
      if (acceptServer) {
        final raw = Map<String, dynamic>.from(row);
        _writeRawToDisk(raw, uid: uid); // bind to uid
        _raw = raw;
        state = AsyncData(_userProfileFromMap(raw));
      }
    });
  ...
  void _startConnectivityListener() {
    ...
    _connSub = Connectivity().onConnectivityChanged.listen((_) async {
      // Flush outbox only if signed in and with coalescing.
      final uid = _uid();
      if (uid == null) return;
      await _flushOutbox();
    });
  ...
  void _startAuthListener() {
    ...
    _authSub = _db.auth.onAuthStateChange.listen((s) async {
      ...
      if (e == AuthChangeEvent.signedOut) {
        ...
        // If the user changed, ensure a clean slate before bootstrapping.
        final uid = _uid();
        final lastUid = _prefs?.getString(_kLastUidKey);
        if (uid != null && lastUid != uid) {
          await clearLocal();
        ...
        _startProfileStream();
        unawaited(_refreshFromServer());
        // Try any pending writes now that we're signed in again.
        unawaited(_flushOutbox());
      }
      // Ignore other events (userUpdated, tokenRefreshed, etc.) to avoid extra calls.
    });
