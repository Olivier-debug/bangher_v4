...
// - Serves cached values instantly (per-user bound; no cross-user leaks)
// - Optimistic updates + outbox queue (retry on connectivity changes)
// - Live sync via Postgrest .stream() (only while signed-in)
// - Clears local state on auth sign-out / user change
// - No manual token refreshes; minimal network; coalesced refresh/flush
//
...
const _kPrefsLastUidKey = 'my_prefs_last_uid'; // bind cache to uid
...
class MyPreferencesStore extends AsyncNotifier<UserPreferences?> {
  ...
  // Throttling (keeps it fast & quiet)
  ...
  Future<UserPreferences?> build() async {
    ...
    if (uid != null && lastUid == uid) {
      ...
      await _clearLocal(); // ensures no stale data leaks
  ...
  Future<void> updatePreferences(Map<String, dynamic> patch) async {
    ...
    _writeRawToDisk(nextRaw, uid: uid); // bind to current user
    _raw = nextRaw;
  ...
  void _writeRawToDisk(Map<String, dynamic> raw, {required String uid}) {
    ...
    _prefs?.setString(_kPrefsLastUidKey, uid);
  }
  ...
  Future<void> _refreshFromServer() async {
    ...
    try {
      ...
      if (acceptServer) {
        ...
        _writeRawToDisk(raw, uid: uid);
        _raw = raw;
        state = AsyncData(_fromRaw(raw));
      }
    } catch (_) {
      // ignore (keep stale data; SWR will try again later)
    } finally {
  ...
  void _startAuthListener() {
    ...
    _authSub = _db.auth.onAuthStateChange.listen((s) async {
      ...
      if (e == AuthChangeEvent.signedOut) {
        ...
        final lastUid = _prefs?.getString(_kPrefsLastUidKey);
        if (uid != null && lastUid != uid) {
          await _clearLocal(); // ensure clean slate for a different user
        }
        _startPrefsStream();
        unawaited(_refreshFromServer());
        unawaited(_flushOutbox());
      }
      // Ignore other events to avoid extra calls.
    });
    ...
    try {
      switch (a.type) {
        case 'update_prefs':
          ...
          patch['user_id'] = uid; // ensure for insert path
          ...
          if (updated.isEmpty) {
            try {
              ...
              if (e.code != '23505') rethrow; // ignore unique violation race
            }
          }
          return true;
