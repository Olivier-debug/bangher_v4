...
//
// CHANGESET A: Cross-fade skeleton → content using AnimatedSwitcher (no renames)
// CHANGESET B: Dependency-free shimmer skeletons; see bottom for reusable
//              SwipeStackSkeleton and ChatListSkeleton (copy to a shared file).
...
class _ViewProfilePageState extends State<ViewProfilePage> {
  ...
  Future<void> _load() async {
    try {
      ...
      if (!mounted) return;
      debugPrint('ViewProfilePage load error: $e');
      setState(() {
        _error = e;
        _loading = false;
      });
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        ...
        return ShaderMask(
          shaderCallback: (rect) {
            final dx = rect.width;
            final double x = (2 * dx) * t - dx; // sweep across
  ...
  Widget build(BuildContext context) {
    return Wrap(
      ...
      children: List.generate(count, (i) {
        final w = 60 + (i * 12 % 100); // varied widths
...
// ===========================================================================
// OPTIONAL (B): Reusable skeletons for other screens (copy to a shared file)
// SUGGESTED PATH: lib/widgets/shimmer_skeletons.dart
// ===========================================================================

// Swipe stack placeholder (top-of-stack card + two behind)
class SwipeStackSkeleton extends StatelessWidget {
  const SwipeStackSkeleton({super.key});
        ...
        return Stack(
          ...
          children: [
            Transform.translate(
              ...
              child: _skeletonCard(cardW * .96, cardH * .96, 16),
            ),
            Transform.translate(
              offset: const Offset(0, 12),
              child: _skeletonCard(cardW * .98, cardH * .98, 17),
            ),
            _skeletonCard(cardW, cardH, 18),
          ],
        );
      },
    );
  ...
  Widget _skeletonCard(double w, double h, double r) {
    return SizedBox(
      width: w,
      height: h,
      child: ClipRRect(
        ...
        child: Stack(
          children: const [
            Positioned.fill(child: _ShimmerBox()),
            Positioned(
              left: 12,
              right: 12,
              bottom: 14,
              child: _ShimmerLine(height: 18, widthFactor: .45),
            ),
          ],
        ),
      ),
...
class ChatListSkeleton extends StatelessWidget {
  const ChatListSkeleton({super.key, this.items = 10});
  final int items;
  @override
  Widget build(BuildContext context) {
    return ListView.separated(
      ...
      itemCount: items,
      separatorBuilder: (_, __) => const Divider(height: 1, color: Color(0xFF26282E)),
      itemBuilder: (_, i) {
        return Padding(
            ...
            children: const [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _ShimmerLine(height: 14, widthFactor: .5),
                    SizedBox(height: 6),
                    _ShimmerLine(height: 12, widthFactor: .9),
                  ],
                ),
              ),
            ],
...
// =========================
// USAGE HINTS (apply in your other files)
// =========================
// 1) Swipe stack page: when initializing with no cards, replace the stack
//    with SwipeStackSkeleton inside _buildStackAndMeasure:
//
//    if (_cards.isEmpty && _fetching) {
//      return const SwipeStackSkeleton();
//    }
//
// 2) Chat list page: while loading chats, show:
//
//    body: const ChatListSkeleton(items: 12),
//
// No renames required; these are purely UI placeholders.
