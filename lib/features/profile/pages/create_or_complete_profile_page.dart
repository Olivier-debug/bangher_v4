...
// ⬇️ cache wiper integration
import '../../../core/cache_wiper.dart';

// ⬇️ NEW: batch picker→cropper→uploader flow
import '../../profile/photo_batch_flow.dart'; // <-- add this import
...
void clearCreateFlowSignedUrlCache() => _SignedUrlCache.clear();
...
class _CreateOrCompleteProfilePageState
  ...
  // ───────── NEW: location mode & dropdown/search state ─────────
  ...
  Future<bool> _safeUpdateProfile(Map<String, dynamic> patch) async {
    if (!mounted) return false; // widget gone
    if (Supabase.instance.client.auth.currentUser == null) return false; // signed out

    try {
      final n = ref.read(profile_facade.myProfileProvider.notifier);
      if (!n.mounted) return false;
      await Future.sync(() => n.updateProfile(patch)); // works for sync/async
      return true;
    } catch (_) {
      return false;
    }
  ...
  Future<bool> _safeRefreshMyProfile() async {
    if (!mounted) return false;
  ...
  void initState() {
    ...
    // Keep the rest of your existing initState code,
    // but REPLACE the subscription with this:
    _authSub = Supabase.instance.client.auth.onAuthStateChange.listen((data) {
      // Only react on logout; do not touch on signedIn / userUpdated.
      if (data.event == AuthChangeEvent.signedOut) {
        // WHY: This page may already be disposed during route changes.
        if (!mounted) return;
  ...
  void didChangeDependencies() {
    ...
    // derive from constructor OR extras OR query params
  ...
  void _clearAllProfileState() {
    ...
    if (!mounted) {
      // still reset draft even if widget unmounted
      _saveDraft();
      return;
    }
    // Provider calls may still race with disposal of ProviderScope → try/catch.
    try { ref.invalidate(profile_facade.myProfileProvider); } catch (_) {}
    try { ref.invalidate(_completionProvider); } catch (_) {}
    try { ref.read(profileStatusListenableProvider).value = ProfileStatus.incomplete; } catch (_) {}
  ...
  bool _pageNeedsKeyboard(_StepId id) {
    switch (id) {
      ...
      case _StepId.about:
        return true; // name, bio, love language
      case _StepId.city:
        // Only when dropdown is open (search field visible)
  ...
  Future<void> _captureLocation() async {
    try {
      ...
      // Ensure dropdown hidden & keyboard closed
  ...
  Future<void> _onAddPhotosMulti() async {
    // Respect max of 6 photos in total.
    ...
    try {
      // Show the batch flow sheet (handles pick → crop each → upload)
      final res = await pickCropAndUploadPhotos(
        context, // ignore: use_build_context_synchronously
        userId: _meId(),
        maxCount: remaining,
        // We store storage paths; resolver signs them on demand.
        returnSignedUrls: false,
      );
      if (!mounted || res == null) return;

      // Merge into first open slots
      final newPaths = res.storagePaths.where((p) => (p.trim().isNotEmpty)).toList();
      ...
      setState(() {
        for (final sp in newPaths) {
          ...
          if (slot == null) break;
          _pictures[slot] = sp; // e.g. "profile_pictures/<uid>/file.jpg"
        }
        _photoBusy = true;
        _photoBusyMsg = 'Saving photos…';
      });

      // Persist to DB and notify local profile facade
                      ...
                      child: Container(
                        ...
                        child: PageView.builder(
                          ...
                          onPageChanged: (i) {
                            ...
                            // NEW: close keyboard if next page doesn't need it
                  ...
                  Padding(
                    ...
                    child: _FooterButtons(
                      ...
                      onPrimary: () async {
                        ...
                        // NEW: always hide keyboard when stepping
  ...
  bool _canProceedForStep(_StepId id) {
    switch (id) {
      ...
      case _StepId.city:
        // Must be exactly one mode with coords set
  ...
  Future<void> _onSave() async {
    ...
    try {
      ...
      // Optimistic local update — guard
      await _safeUpdateProfile(profileMap);

      // Upsert to DB (profiles/preferences) — unchanged
      ...
      // Invalidate cached profile — guard
  ...
  // ──────────── REPLACE the _buildCityStep with this upgraded version ────────────
  Widget _buildCityStep(BuildContext context) => _buildPadding(
        child: Column(
          ...
          children: [
            ...
            const Text(
              'Pick exactly one: use your current location, or choose a city.',
            ...
            ElevatedButton.icon(
              onPressed: () async {
                _locDropdownOpen = false; // dropdown must disappear
                setState(() {}); // refresh visibility
                final ok = await _ensureLocationPermission();
                if (!ok) return;
                HapticFeedback.selectionClick();
                await _captureLocation(); // sets _location2 and _city label
                if (!mounted) return;
            ...
            OutlinedButton.icon(
              onPressed: () {
                HapticFeedback.selectionClick();
                setState(() {
                  // Open/close dropdown; clear GPS selection when opening
                  _locDropdownOpen = !_locDropdownOpen;
                  if (_locDropdownOpen) {
                    _locationMode = _LocationMode.manual;
                    _location2 = null; // clear GPS pins until user picks
                  }
                });
                _scheduleSaveDraft();

                // Focus the search only when opening
                if (_locDropdownOpen) {
                  // Just ensure no stray keyboard visible
            ...
            Wrap(
              ...
              children: [
                if (_locationMode == _LocationMode.gps && _location2 != null)
                  _okChip('Using your current location'),
                if (_locationMode == _LocationMode.manual && _city.text.trim().isNotEmpty)
                  _okChip('Chosen: ${_city.text.trim()}'),
              ],
            ),
                                    ...
                                    return ListTile(
                                      ...
                                      onTap: () {
                                        ...
                                        setState(() {
                                          ...
                                          _locDropdownOpen = false; // hide dropdown
                                        });
                                        // Ensure GPS selection cannot coexist
                                        FocusManager.instance.primaryFocus?.unfocus();
        ...
        child: Column(
          ...
          children: [
            ...
            _EditStylePhotosGrid(
              pictures: _nonNullPictures(),
              // ⬇️ CHANGED: use multi-select flow on the "+" tile
...
class _InputText extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Prevent focus entirely when read-only, so no keyboard ever shows.
