...
// NEW: expose cache invalidation helper from router
import '../../../routing/router.dart' show invalidateProfileStatusCache;

// NEW: optimistic profile store (import only)
...
class _CreateOrCompleteProfilePageState extends ConsumerState<CreateOrCompleteProfilePage> {
  ...
  // Reused HTTP client (3)
  late final http.Client _http;

  // NEW: auth-state subscription so we can clear caches on user create/sign-in/out
  ...
  void initState() {
    ...
    // Validate steps live as user types (About/Love language & City too)
    ...
    // NEW: skip the completeness gate for brand-new signups & start clean.
    if (widget.fresh) {
    ...
    // (1) Redirect moved out of build: listen to completion and navigate
    if (!widget.fresh) {
    ...
    // NEW: clear profile caches/state on auth changes (sign in/out/switch)
  ...
  // Respond if router rebuilds this page with fresh=true after first mount
  @override
  ...
  // NEW: one-stop cleanup for all local fields + provider caches
  void _clearAllProfileState() {
    ...
    // Invalidate Riverpod caches + router status
  ...
  Future<void> _putRowByUserId({
    ...
    // 1) Try update first (works if row already exists)
    ...
    // 2) Insert (if another client won the race, ignore unique violation)
    try {
  ...
  Future<void> _onAddPhotoNextOpen() async {
    ...
    try {
      ...
      if (slot == null) {
        ...
        // ALSO update local store optimistically so UI elsewhere is instant
  ...
  Future<void> _onSave() async {
    ...
    try {
      ...
      map['user_id'] = me;
      // Now that everything validates, mark complete for fast-path checks.
      map['complete'] = true;
      ...
      // Optimistic local update BEFORE remote upserts (queued via store outbox)
      ...
      // IMPORTANT: tell the router to forget the old status
      invalidateProfileStatusCache();

      // Then jump straight to the main app (no redirect flash)
      if (mounted) {
        context.go(TestSwipeStackPage.routePath);
      }
  ...
  Widget build(BuildContext context) {
    ...
    if (!widget.fresh && user != null) _prefillOnce(profile);
    if (!widget.fresh && user != null && !_prefilledPrefs) _prefillInterestedIn();
    ...
    if (!widget.fresh) {
      ...
      if (completion.complete) {
        // Redirect handled by ref.listen in initState (1)
        return _GateScaffold(
          title: 'All set! ✅',
            ...
            SafeArea(
              ...
              child: Column(
                children: [
                  ...
                  // (2) Footer extracted to reduce rebuild scope
                  Padding(
                    padding: const EdgeInsets.fromLTRB(16, 0, 16, 16),
      ...
      final picked = await showDatePicker(
        ...
        builder: (context, child) => Theme(
          data: Theme.of(context).copyWith(
            ...
            dialogTheme: const DialogThemeData(backgroundColor: AppTheme.ffPrimaryBg),
          ),
          child: child!,
        ),
      );
