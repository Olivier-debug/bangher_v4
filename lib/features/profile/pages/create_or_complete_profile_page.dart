...
class _CreateOrCompleteProfilePageState extends ConsumerState<CreateOrCompleteProfilePage> {
  ...
  Future<void> _onAddPhotoNextOpen() async {
    ...
    try {
      ...
      if (slot == null) {
        ...
        await ref.read(myProfileStoreProvider.notifier).setPhotos(_nonNullPictures()); // why: keep store in sync
      }
    } catch (e) {
  ...
  Future<void> _onSave() async {
    ...
    try {
      ...
      // NEW: optimistic local update BEFORE remote upserts (queued via store outbox)
      await ref.read(myProfileStoreProvider.notifier).updateProfile(map); // why: instant UI + offline tolerant
                          ...
                          child: OutlinedButton(
                            onPressed: _index == 0
                                ...
                                : () async {
                                    ...
                                    if (!context.mounted) return; // guard the local BuildContext
  ...
  Widget _buildPhotosAndPrefsStep(BuildContext context) => _buildPadding(
        child: Column(
          ...
          children: [
            ...
            // NOTE: ensure RangeSlider import and Theme are ok in your project.
            RangeSlider(
              values: _ageRange,
              min: 18,
              max: 100,
