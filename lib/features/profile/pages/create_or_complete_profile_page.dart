...
// Applied same improvements as other pages:
// - AUTH-SAFE: Early return if not signed in (no provider/network churn)
// - Signed image loader with short-lived Supabase signed URLs + tiny TTL cache
//   and cacheWidth hints for faster, lighter images (web & mobile)
// - Crop dialog UX parity with edit page; safe upload (web + mobile temp file)
// - Unified inputs/chips styling (charcoal fill, AppTheme.ffAlt outline, pill-10)
// - Keeps existing logic for completion RPC, store, and navigation.
...
class _SignedImage extends StatelessWidget {
  ...
  final int? explicitCacheWidth; // optional exact cacheWidth (device px)
...
final _completionProvider = FutureProvider<_ProfileCompletion>((ref) async {
  ...
  final row = await client.from('profiles').select('complete').eq('user_id', uid).maybeSingle();
  ...
  // Not complete: precise evaluation (single RPC)
...
class CreateOrCompleteProfilePage extends ConsumerStatefulWidget {
  ...
  /// When true, user just signed up → skip the gate screens and start full flow.
  final bool fresh;
...
class _CreateOrCompleteProfilePageState extends ConsumerState<CreateOrCompleteProfilePage> {
  ...
  void initState() {
    ...
    // Validate steps live as user types
    ...
    // Fresh signup: start full flow and clear any cached state
    if (widget.fresh) {
    ...
    // Redirect listener (only when authenticated and not fresh)
    final isAuthed = Supabase.instance.client.auth.currentUser != null;
    if (!widget.fresh && isAuthed) {
      ref.listen<AsyncValue<_ProfileCompletion>>(
        _completionProvider,
        (prev, next) {
          final done = next.valueOrNull?.complete ?? false;
          if (done && mounted) {
            context.go(TestSwipeStackPage.routePath);
          }
        },
      );
    }

    // Clear profile caches/state on auth changes (sign in/out/switch)
  ...
  String? _mapInterestedInToDb(String? ui) {
    switch ((ui ?? '').toLowerCase()) {
      ...
      case 'both':
        return 'O';
      default:
        return ui?.isEmpty ?? true ? null : ui;
    }
  }
  ...
  void _clearAllProfileState() {
    ...
    ref.invalidate(myProfileStoreProvider);
  ...
  String? _blockReason(_StepId id) {
    switch (id) {
      ...
      case _StepId.city:
        return 'Set your city or use your location to continue.';
      case _StepId.about:
  ...
  Future<void> _putRowByUserId({
    ...
    // Try update first
    final updated = await client.from(table).update(payloadWithUserId).eq('user_id', userId).select('user_id');
    if (updated.isNotEmpty) return;

    // Insert; ignore unique violation if racing
    try {
  ...
  Widget _buildStorePeek(AsyncValue<UserProfile?> storeAsync) {
    final sp = storeAsync.valueOrNull;
    final photos = sp?.profilePictures.length ?? 0;
    final name = sp?.name ?? '-';
    return Container(
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text('Store → name: $name | photos: $photos', style: const TextStyle(fontSize: 11, color: Colors.white70)),
        const SizedBox(width: 6),
        InkWell(
          onTap: () => ref.read(myProfileStoreProvider.notifier).refresh(),
          child: const Padding(
  ...
  // ---- Crop dialog (parity with edit page) ----
  ...
  Future<XFile?> _pickWithPermissions(ImageSource source) async {
    try {
      ...
      _snack('Unable to open ${source == ImageSource.camera ? 'camera' : 'gallery'}: ${e.message ?? e.code}',
          isError: true);
      return null;
    } catch (e) {
  ...
  Future<void> _onAddPhotoNextOpen() async {
    ...
    try {
      ...
      if (slot == null) {
        ...
        await ref.read(myProfileStoreProvider.notifier).setPhotos(_nonNullPictures());
      }
    } catch (e) {
  ...
  Future<void> _onSave() async {
    ...
    try {
      ...
      final update = ProfileUpdate(
        ...
        currentCity: _city.text.trim().isNotEmpty ? _city.text.trim() : null,
        bio: _bio.text.trim().isNotEmpty ? _bio.text.trim() : null,
        loveLanguage: _loveLanguage.text.trim().isNotEmpty ? _loveLanguage.text.trim() : null,
        dateOfBirth: _dob,
      ...
      map['user_id'] = me;
      // Mark complete for fast-path checks.
      map['complete'] = true;
      ...
      // Optimistic local update BEFORE remote upserts
      await ref.read(myProfileStoreProvider.notifier).updateProfile(map);
  ...
  Widget build(BuildContext context) {
    ...
    // Safe to read providers.
    final profileAsync = ref.watch(myProfileProvider);
    final profile = profileAsync.valueOrNull;

    // NEW: don't prefill when fresh, so the new user starts from scratch
    if (!widget.fresh) _prefillOnce(profile);
    ...
    if (!widget.fresh) {
      ...
      if (completion.complete) {
        // Redirect handled by ref.listen in initState
        return _GateScaffold(
          title: 'All set! ✅',
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: _showDebugStorePeek
              ? [Padding(padding: const EdgeInsets.only(right: 8), child: _buildStorePeek(ref.watch(myProfileStoreProvider)))]
              : null,
        ),
        body: Stack(
          children: [
  ...
  Widget _buildNameStep(BuildContext context) => _buildPadding(
        child: Column(
          ...
          children: [
            ...
            _InputText(label: 'First name', controller: _name, hint: 'Enter your first name', required: true),
            const SizedBox(height: 16),
  ...
  Widget _buildAboutStep(BuildContext context) => _buildPadding(
        child: Column(
          ...
          children: [
            ...
            _InputText(label: 'Short bio', controller: _bio, hint: 'Tell people a little about you', maxLines: 4),
            const SizedBox(height: 12),
            _InputText(label: 'Love language', controller: _loveLanguage, hint: 'e.g. Quality Time'),
          ],
        ),
      );
...
// Unified input styling (matches EditProfile/UserProfile tokens)
