...
class _CreateOrCompleteProfilePageState extends ConsumerState<CreateOrCompleteProfilePage> {
  ...
  void initState() {
    ...
    // FIX: validate steps live as user types (About/Love language & City too)
    ...
    // NEW: skip the completeness gate for brand-new signups.
    if (widget.fresh) {
      _choice = FlowStartChoice.all;
      _activeSteps = List.of(_allSteps);
      _index = 0;
      // PageView will render immediately in build()
    }
  }

  // FIX: respond if router rebuilds this page with fresh=true after first mount
  @override
  ...
  void _onFieldChanged() {
    // ultra-cheap diff could be added here; setState() is fine (single widget tree)
    if (mounted) setState(() {});
  }
  ...
  Future<void> _onSave() async {
    ...
    try {
      ...
      // Ensure complete=true on row (if not already updated by upsert)
