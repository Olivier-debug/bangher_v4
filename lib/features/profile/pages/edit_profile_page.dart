...
// Offline-first prefill: reads PeerProfileCache first; renders form even offline.
// On save success, writes back to PeerProfileCache for future offline loads.
...
// NEW: local-first cache for hydration
...
class _EditProfilePageState extends ConsumerState<EditProfilePage> {
  ...
  // One-time prefill guard (from cache OR provider)
  ...
  void initState() {
    super.initState();
    _hydrateFromLocalFirst(); // offline-first prefill
  }
  ...
  void _prefillFromRaw(Map<String, dynamic> m) {
    String? _str(dynamic v) => (v?.toString().trim().isEmpty ?? true) ? null : v.toString().trim();
    List<String> _list(dynamic v) =>
        v is List ? v.map((e) => e?.toString() ?? '').where((s) => s.trim().isNotEmpty).cast<String>().toList() : [];

    _name.text = _str(m['name']) ?? '';
    _city.text = _str(m['current_city']) ?? '';
    _bio.text = _str(m['bio']) ?? '';
    _loveLanguage.text = _str(m['love_language']) ?? '';
    _communicationStyle.text = _str(m['communication_style']) ?? '';
    _education.text = _str(m['education']) ?? '';
    _familyPlans.text = _str(m['family_plans']) ?? '';

    _gender = _fromDbGender(_str(m['gender']));
    final dobStr = _str(m['date_of_birth']);
    _dob = dobStr != null ? DateTime.tryParse(dobStr) : null;

    _pictures
      ..clear()
      ..addAll(_list(m['profile_pictures']));

    _interests
      ..clear()
      ..addAll(_list(m['interests']));

    _relationshipGoals
      ..clear()
      ..addAll(_list(m['relationship_goals']));

    // Accept either 'languages' or 'my_languages'
    final langs = _list(m['my_languages'])..addAll(_list(m['languages']));
    _languages
      ..clear()
      ..addAll(langs.toSet());

    _drinking = _str(m['drinking']);
    _smoking = _str(m['smoking']);
    _pets = _str(m['pets']);

    _heightCm = (m['height_cm'] as num?)?.toInt();
    _zodiac = _str(m['zodiac_sign']);
    _workout = _str(m['workout']);
    _dietary = _str(m['dietary_preference']);
    _sleeping = _str(m['sleeping_habits']);
    _sexualOrientation = _str(m['sexual_orientation']);
    _socialMedia.text = _str(m['social_media']) ?? '';
    _personalityType.text = _str(m['personality_type']) ?? '';
  ...
  Widget build(BuildContext context) {
    ...
    // Provider (may be offline). If we haven't prefilled yet and data arrives, prefill once.
    ...
    if (!_prefilled && p != null) {
      ...
      _pictures
        ..clear()
        ..addAll(p.profilePictures);
      _interests
        ..clear()
        ..addAll(p.interests);
      _relationshipGoals
        ..clear()
        ..addAll(p.relationshipGoals);
      _languages
        ..clear()
        ..addAll(p.languages);
        ...
        children: [
          SafeArea(
            child: profileAsync.when(
              // If we already hydrated from cache, render the form even while loading.
              loading: () => _prefilled ? form() : const Center(child: CircularProgressIndicator()),
              // On error, still allow editing with cached values if any.
              error: (e, _) => _prefilled
                  ? form()
                  : Center(
                      child: Padding(
                        padding: const EdgeInsets.all(24),
  ...
  Future<void> _onSave() async {
    ...
    try {
      ...
      final map = <String, dynamic>{
        ...
        // NOTE: keep both keys so other screens reading 'languages' still work offline
      ...
      // Persist to local cache so the page boots with this data offline next time.
      await PeerProfileCache.instance.write(me, map);
