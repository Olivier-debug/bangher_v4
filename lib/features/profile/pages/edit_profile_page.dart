...
class _EditProfilePageState extends ConsumerState<EditProfilePage> {
  ...
  bool _prefilled = false; // one-time prefill guard
  ...
  void initState() {
    ...
    // Resume any pending photo uploads quietly.
  ...
  void _prefillFromRaw(Map<String, dynamic> m) {
    ...
    final dobStr = str(m['date_of_birth']);
    ...
    final langs = listStr(m['my_languages'])..addAll(listStr(m['languages']));
    _languages
      ..clear()
      ..addAll(langs.toSet());
  ...
  Widget build(BuildContext context) {
    ...
    final profileAsync = ref.watch(myProfileStoreProvider);
  ...
  Future<Uint8List?> _cropWithDialogPro(Uint8List srcBytes) async {
    ...
    await showDialog(
      ...
      builder: (ctx) {
        const double aspect = 4 / 5; // enforce consistent aspect across photos
        bool busy = false;
                      ...
                      _pillButton(
                        ...
                        onTap: () async {
                          ...
                          await _withPhotoBusy('Deleting photo…', () async {
                            ...
                            await PeerProfileCache.instance.write(uid, raw);
                            // reflect optimistically in store
                            await ref
                                .read(myProfileStoreProvider.notifier)
  ...
  Future<void> _replacePhotoAt(int index) async {
    ...
    try {
      final entryId = await _PhotoUploadOutbox.instance.enqueueReplace(
        ...
        index: index,
        fileName: xfile.name,
        bytes: kIsWeb ? croppedBytes : null,
        tempPath: !kIsWeb
            ? await tempfs.saveBytesToTempFile(
                croppedBytes,
                'p_${DateTime.now().millisecondsSinceEpoch}_${xfile.name.replaceAll(RegExp(r'[^A-Za-z0-9._-]+'), '_')}',
              )
            : null,
      );
  ...
  Future<void> _onSave() async {
    ...
    try {
      ...
      final patch = <String, dynamic>{
        ...
        // keep both keys to serve older readers offline
      ...
      // Optimistic local write + queue remote via store
      await ref.read(myProfileStoreProvider.notifier).updateProfile(patch);
      ...
      // Nudge a background refresh (why: reconcile with server after flush)
      await ref.read(myProfileStoreProvider.notifier).refresh();
...
class _PhotoUploadOutbox {
  ...
  static final instance = _PhotoUploadOutbox._();
  ...
  Future<void> flush({
    ...
    try {
      ...
      // derive current pictures
      List<String> current = [];
      final storeProfile = ref.read(myProfileStoreProvider).valueOrNull;
      if (storeProfile != null) {
        current = List<String>.from(storeProfile.profilePictures);
      } else {
      ...
      for (final e in queue) {
        try {
          final String? tempPath = (e['tempPath'] as String?);
          ...
          if (kIsWeb || tempPath == null) {
            ...
            url = await repo.uploadProfileImage(userId: userId, filePath: fileName, bytes: bytes);
          } else {
            url = await repo.uploadProfileImage(userId: userId, filePath: tempPath);
          }
          ...
          await ref.read(myProfileStoreProvider.notifier).updateProfile({'profile_pictures': current});
          anyChange = true;
        } catch (_) {
          nextQueue.add(e); // keep
        }
      }

      if (anyChange) {
        await onPicturesUpdated(current);
      }
