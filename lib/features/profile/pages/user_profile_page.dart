...
// Changes for wiring:
// - Swapped provider import to `providers.dart` (aliases store providers).
// - Added lifecycle-based refresh on resume for extra snappiness.
// - Added pull-to-refresh (calls store.refresh()).
...
import '../providers.dart'; // <── use store aliases
...
class _SignedUrlCache {
  ...
  static Future<String> resolve(String urlOrPath) async {
    ...
    if (i <= 0) throw StateError('Invalid storage path: $urlOrPath');
...
class _UserProfilePageState extends ConsumerState<UserProfilePage> with WidgetsBindingObserver {
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    // Why: make it feel instant after coming back from edit page / background
    if (state == AppLifecycleState.resumed) {
      // fire-and-forget; SWR will avoid extra work if recently refreshed
      // Prefer store refresh (no network if signed-out)
  ...
  Widget build(BuildContext context) {
    ...
    // Normal path (signed-in): safe to read your provider (store-backed SWR).
        ...
        body: SafeArea(
          ...
          child: hasError
              ...
              : (isLoading && p == null)
                  ...
                  // We have something → progressively render sections
                  : RefreshIndicator(
                  ...
                  child: Hero(
                    ...
                    child: FutureBuilder<String>(
                      ...
                      builder: (context, snap) {
                        final url = snap.data;
                        if (url == null) {
                          return const _ShimmerBox(); // quick placeholder while signing
                        }
                    ...
                    return GestureDetector(
                      ...
                      child: Hero(
                        ...
                        child: FutureBuilder<String>(
                          ...
                          builder: (context, snap) {
                            final url = snap.data;
                            if (url == null) return const _ShimmerBox(); // shimmer while signing/resolving
