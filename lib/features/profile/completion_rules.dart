...
// Matches server-side RPC: requires bio + love_language, >=1 language, >=1 goal,
// >=3 interests, >=1 photo, valid prefs, city OR GPS (location2).
...
ProfileCompletion evaluateProfileCompletion({
  ...
  List<String> strList(dynamic v) => (v as List?)?.whereType<String>().toList() ?? const [];
  ...
  if (dobStr != null && dobStr.isNotEmpty) {
    try { dob = DateTime.parse(dobStr); } catch (_) {}
  }
  bool over18 = false;
  if (dob != null) {
    var years = now0.year - dob.year;
    final hadBirthday = (now0.month > dob.month) || (now0.month == dob.month && now0.day >= dob.day);
  ...
  final cityOk = _isNonEmptyCity(str(profile?['current_city'])) || _hasLocation2(profile?['location2']);
  ...
  // interests >= 3
  if (strList(profile?['interests']).length < 3) missing.add(StepId.interests);

  // goals >= 1
  if (strList(profile?['relationship_goals']).isEmpty) missing.add(StepId.goals);

  // languages >= 1
  if (strList(profile?['my_languages']).isEmpty) missing.add(StepId.languages);
  ...
  if (photos.isEmpty || !prefsOk) missing.add(StepId.photosAndPrefs);
