...
Future<BatchUploadResult?> pickCropAndUploadPhotos(
  ...
  final picked = await _pickMany(context, maxCount: maxCount);
...
Future<List<(String, Uint8List)>> _pickMany(BuildContext context, {required int maxCount}) async {
  try {
    if (!kIsWeb) {
  ...
  Future<void> _runPipeline() async {
    ...
    for (int i = 0; i < widget.items.length; i++) {
      ...
      final cropped = await showProfilePhotoCropper(
        context,
        sourceBytes: it.bytes,
        aspectRatio: 4 / 5,
        jpegQuality: 92,
      );
      if (!mounted) break; // guard after async gap
      if (_cancelled) break;
      ...
      // Upload
      setState(() => it.state = _ItemState.uploading);
      ...
      try {
        ...
        if (widget.returnSignedUrls) {
          final url = await client.storage.from(widget.bucket).createSignedUrl(
                path,
                widget.signedExpiry.inSeconds,
              );
          it.url = url;
        } else {
          it.url = client.storage.from(widget.bucket).getPublicUrl(path);
        }
    ...
    if (!_cancelled) {
      ...
      if (mounted) {
        Navigator.of(context).pop(BatchUploadResult(
          storagePaths: paths,
