...
import 'confession_detail_page.dart';
          ...
          child: _loading
              ...
              : StretchingOverscrollIndicator(
                  ...
                  child: CustomScrollView(
                    controller: _scroll,
                    slivers: [
                      // Header with logo/options removed (belongs in router)
                      SliverToBoxAdapter(
                        child: Column(
                          children: [
                            _FiltersRow(
                              ...
                              onConfess: () async {
                                final created = await showModalBottomSheet<ConfessionItem>(
                                  context: context,
                                  isScrollControlled: true,
                                  useSafeArea: true,
                                  backgroundColor: Colors.transparent,
                                ...
                                if (created != null) {
                                  ...
                                  FeedCache.instance.upsert(created);
                                }
                              },
                            ),
        ...
        return _ConfessionCard(
          ...
          onOpenComments: () async {
            await showModalBottomSheet<void>(
              ...
              useSafeArea: true,
              backgroundColor: Colors.transparent,
              builder: (_) => CommentsSheet(confessionId: item.id),
            );
          },
          onBookmark: () async {
          ...
          onOpenDetail: () {
            Navigator.of(context).push(MaterialPageRoute(
              builder: (_) => ConfessionDetailPage(confessionId: item.id),
              settings: const RouteSettings(name: ConfessionDetailPage.routePath),
            ));
          },
        );
      },
...
/// UI bits (AppBar / Filters / Skeleton / Card) -------------------------------
...
class _ConfessionCardState extends State<_ConfessionCard> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: InkWell(
        onTap: widget.onOpenDetail, // whole card opens detail
        borderRadius: BorderRadius.circular(16),
        child: Container(
          decoration: BoxDecoration(
            color: AppTheme.ffPrimaryBg,
