...
// REPLACED: old chat list import
// import '../features/chat/page/chat_list_page.dart';
import '../features/inbox/inbox_page.dart'; // <— new 3-tab inbox
...
final Provider<GoRouter> routerProvider = Provider<GoRouter>((ref) {
  ...
  return GoRouter(
    ...
    routes: <RouteBase>[
      ...
      GoRoute(
        ...
        pageBuilder: (context, state) => NoTransitionPage(
          child: _AppScaffold(
            ...
            currentLocation: state.matchedLocation,
            showHeader: true,
            headerVariant: _HeaderVariant.confessions,
          ),
        ),
        routes: [
          // /confessions/detail?cid=123
          GoRoute(
          ...
          // /confessions/compose (fullscreen)
          GoRoute(
      ...
      // REPLACED: ChatListPage route -> InboxPage route
      GoRoute(
      ...
      // ───────── Chat detail (single source of truth) ─────────
      // Use `extra` to pass the matchId (int or map). No alias route, no duplicate names.
      GoRoute(
        path: ChatPage.routePath, // '/chat'
        name: ChatPage.routeName, // 'chat'
        builder: (context, state) {
          int matchId = 0;
          ...
          if (extra is int) {
            matchId = extra;
          } else if (extra is Map && extra['matchId'] != null) {
            final v = extra['matchId'];
            matchId = v is int ? v : int.tryParse(v.toString()) ?? 0;
          } else {
            // Optional fallback: /chat?id=123
            final idStr = state.uri.queryParameters[_kIdParam];
            matchId = int.tryParse(idStr ?? '') ?? 0;
          }
...
enum _HeaderVariant { swipe, profile, confessions }
...
class _AppScaffold extends StatelessWidget {
      ...
      child: Scaffold(
        ...
        bottomNavigationBar: AppBottomNav(
          ...
          items: [
            ...
            // REPLACED: ChatListPage tab -> InboxPage tab
            const NavItem(
              icon: Icons.chat_bubble_outline,
              label: 'Chats',
              path: InboxPage.routePath,
              selectedStartsWith: [InboxPage.routePath, ChatPage.routePath],
            ),
            const NavItem(
  ...
  PreferredSizeWidget _buildHeader(BuildContext context, _HeaderVariant variant) {
    switch (variant) {
      ...
      case _HeaderVariant.confessions:
        return AppBar(
          toolbarHeight: _kHeaderHeight,
          centerTitle: false,
          titleSpacing: 12,
                ...
                messenger
                  ...
                  ..showSnackBar(
                    const SnackBar(
                      ...
                      content: Text('Notifications coming later 🔔'),
                    ),
                  );
              },
            ),
                ...
                onPressed: () {
                  ...
                  messenger
                    ...
                    ..showSnackBar(
                      const SnackBar(
                        behavior: SnackBarBehavior.floating,
                        content: Text('Tip: use the Confess button below'),
                      ),
                    );
                },
