...
final Provider<GoRouter> routerProvider = Provider<GoRouter>((ref) {
  ...
  // Use the guard's ValueNotifier so router refreshes when completeness changes.
  ...
  FutureOr<String?> redirect(BuildContext _, GoRouterState state) async {
    ...
    if (session == null) {
      // Allow unauth pages; otherwise send to login.
      if (atLogin || atOnboarding || atSplash) return null;
      return LoginPageWidget.routePath;
    }

    // While on the create/complete page, never redirect away (the page will decide).
    if (atCreateComplete) return null;

    // If auth’d but on login, decide where to go.
    if (atLogin) {
      ...
      return null; // unknown -> wait
    }

    // Gate all other locations by status.
    final st = profileGateListenable.value;
    if (st == ProfileStatus.incomplete) {
      return CreateOrCompleteProfilePage.routePath;
    }

    // st == complete or unknown -> proceed (unknown happens at app boot for a tick)
    return null;
  }
  ...
  return GoRouter(
    ...
    refreshListenable: profileGateListenable, // refresh when ProfileStatus changes
    redirect: redirect,
    redirectLimit: 2,
    ...
    routes: <RouteBase>[
      ...
      // Tabs (disable transitions so the bottom bar doesn't slide in)
      GoRoute(
