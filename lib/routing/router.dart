// ─────────────────────────────────────────────────────────────────────────────
// FILE: lib/routing/router.dart (patched)
// Purpose of patch: Keep bottom nav from "coming in" on tab switches by
// disabling page transitions for the four tab root routes via NoTransitionPage.
// Also: Robust Android back handling —
//   • Pops normally when possible
//   • If on a non-home tab root, switches to home instead of exiting
//   • On the home tab root, requires double-back within 2s to exit (Android)
...
import 'package:flutter/services.dart'; // SystemNavigator.pop for controlled exit on Android
...
final Provider<GoRouter> routerProvider = Provider<GoRouter>((ref) {
  ...
  return GoRouter(
    ...
    routes: <RouteBase>[
      ...
      // Tabs (with bottom nav) — disable transitions so bottom bar doesn't "come in"
      GoRoute(
      ...
      // Keep Chat detail as-is (normal push animation feels natural inside tab)
      GoRoute(
...
class _AppScaffold extends StatelessWidget {
  ...
  // Guard for double-back-to-exit on Android home tab
  ...
  Widget build(BuildContext context) {
    ...
    // Intercept system back to provide predictable behavior with tabs:
    // 1) If anything can pop, let it pop (dialogs, detail pages).
    // 2) If on a non-home tab root, switch to the home tab instead of exiting.
    // 3) If already on the home tab root, require double back to exit (Android).
    return PopScope(
      canPop: router.canPop(),
      onPopInvokedWithResult: (didPop, result) async {
        if (didPop) return; // something above handled it
        ...
        if (!onHomeTab) {
          // Switch to the home tab root instead of exiting
          if (context.mounted) context.go(TestSwipeStackPage.routePath);
          return;
        }

        // Home tab root → double-back-to-exit on Android only
        ...
        if (platform == TargetPlatform.android) {
          ...
          // Exit to background (best-practice UX for Android back on home)
